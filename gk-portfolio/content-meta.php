	...
	<?php
		...
		if ('post' == get_post_type()) {
			...
			if(get_theme_mod('portfolio_post_show_category', '1') == '1') {
				...
				$categories_list = get_the_category_list(__( ', ', 'gk-portfolio'));
				if ($categories_list) {
					echo '<span class="categories-links">' . __('Posted in ', 'gk-portfolio') . $categories_list . '</span>';
				}
			}
		}
		
		if(current_user_can('edit_posts') || current_user_can('edit_pages')) {
			edit_post_link(__('Edit', 'gk-portfolio'), '<span class="edit-link">', '</span>');
		}
	?>
</aside><!-- .post-meta -->
<?php endif; ?>