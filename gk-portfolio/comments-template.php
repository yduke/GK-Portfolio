...
function portfolio_comment_template($comment, $args, $depth) {
	...
	<li class="pingback">
		<p>
			<?php _e('Pingback:', 'gk-portfolio'); ?> 
			<?php comment_author_link(); ?>
			<?php edit_comment_link(__('Edit', 'gk-portfolio'), '<span class="edit-link">', '</span>'); ?>
		</p>
		<?php break; ?>
	<?php default : ?>
	<?php
		$comment_class = comment_class(empty( $args['has_children'] ) ? '' : 'parent', null, null, false);
		$comment_author = in_array('bypostauthor', explode(' ', $comment_class)) ? ' <strong class="comment-post-author">' . __('Post author', 'gk-portfolio') . '</strong>' : '';
	?>
				...
				<div class="comment-metadata vcard">
					...
					<a href="<?php echo esc_url( get_comment_link( $comment->comment_ID, $args ) ); ?>">
						<time datetime="<?php comment_time( 'c' ); ?>">
							<?php printf( _x( '%1$s at %2$s', '1: date, 2: time', 'gk-portfolio' ), get_comment_date(), get_comment_time() ); ?>
						</time>
					</a>
				</div><!-- .comment-metadata -->

				<?php if ('0' == $comment->comment_approved) : ?>
				<p class="comment-awaiting-moderation"><?php _e('Your comment is awaiting moderation.', 'gk-portfolio'); ?></p>
				<?php endif; ?>
			</footer><!-- .comment-meta -->
